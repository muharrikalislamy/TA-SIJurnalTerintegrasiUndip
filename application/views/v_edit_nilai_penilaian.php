...
<div class="content-wrapper">
      ...
      <div class="box-body">  
        <form class="" action="<?php echo base_url()?>Penilaian_Alternatif/update_nilai_alternatif" method="post">
      ...
      <?php foreach ($indikator as $key => $indi) {?>
        ...
        <div class="form-group">
          ...
          <div class="">
            <select class="form-control" name="nilai[<?=$indi->kode_kriteria?>][]" >
                <?php foreach ($nilai as $k => $value) { ?>
                    <option value="<?=$value->nilai?>" <?=$recordnilai[$key]['nilai'] === $value->nilai ? "selected" : ""?>><?=$value->penilaian?> [<?=$value->nilai?>]</option>
                <?php } ?>
            </select>
          </div>
        </div>
        ...
        <div class="form-group"> <!-- Form Input Catatan -->
            <div class="">
              <textarea class="form-control" rows="3" type="text" name="catatan[<?=$indi->kode_kriteria?>][]" placeholder="Catatan"><?=$recordnilai[$key]['catatan']?></textarea>
            </div>
        </div>
      <?php }?>
